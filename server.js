...
async function guardar(fila) {
  if (!pool) { memoria.push({ ...fila, creado: new Date() }); return; }
  await pool.query(
...
async function leerTodas() {
  ...
  const r = await pool.query(
    'SELECT quien, correo, orden, porque, peor, peor_porque, memoria, libre, creado ' +
    'FROM respuestas ORDER BY id');
  // La base usa snake_case; el resto del código habla camelCase.
  r.rows.forEach(f => { f.peorPorque = f.peor_porque; });
  return r.rows;
}
...
app.get('/api/resultados', async (req, res) => {
  ...
  try {
    res.json(calcular(await leerTodas()));
  } catch (err) {
